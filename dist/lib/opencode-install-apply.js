...
import {
  ...
  validateManagedPath,
} from "./opencode-install-state.js";
...
function writeFlushed(path, bytes, flag = "w") {
...
function isCreatedDirectoryPath(value) {
...
function requireOrdinaryDir(path, label) {
...
function ensureDestinationTree(destination) {
...
function readOwner(lockPath) {
  try {
    ...
    const value = JSON.parse(readFileSync(ownerFile(lockPath), "utf8"));
    if (
      !isRecord(value) ||
...
function createHeldLock(lockPath, token, io) {
  let tombstonePath;
...
function cleanCandidateDebris(deniz) {
...
function releaseOwnedDirectory(path, token, label) {
  if (!existsLstat(path)) {
    return;
  }
...
function acquireReclaimGuard(deniz, io) {
...
export function acquireInstallerLock(destination, options = {}) {
...
function requireHeldLock(lock, destination) {
...
function blocked(transactionDir, message) {
...
function parsePlanOperation(value) {
...
function parseIntent(value) {
  ...
  const intent = { syscall: value.syscall };
  if (typeof value.operationIndex === "number" && Number.isInteger(value.operationIndex) && value.operationIndex >= 0) {
...
function digestOfStateFile(path) {
  try {
...
function observedFileIdentity(path) {
...
function snapshotsDir(transactionDir) {
  return join(transactionDir, "snapshots");
}
function allowedAction(kind, action) {
  if (action === "backed-up") {
...
function createdDirAllowed(dir, operations) {
...
function postCommitPruneCandidates(operations) {
...
function validateJournalSemantics(journal) {
  const seen = new Map();
...
function validateSnapshotTransition(previous, current) {
  ...
  const phaseOrder = { prepared: 0, "files-placed": 1, "state-committed": 2 };
...
function readStateEvidenceFile(path, expectedDigest, label) {
  try {
...
function ownedIdentity(state, path) {
  const owned = state.files[path];
  return owned ? { sha256: owned.sha256, mode: owned.mode } : null;
}
function operationStateError(operation, oldState, newState) {
...
function expectedAppliedMutations(journal, oldState, newState) {
  const paths = new Set();
...
function validateJournalAgainstStates(journal, oldState, newState) {
...
function validatePersistedBackupEvidence(transactionDir, journal, oldState) {
  for (const [operationIndex, operation] of journal.operations.entries()) {
    if (operation.kind !== "replace" && operation.kind !== "remove") {
      continue;
    }
...
function loadLatestSnapshot(transactionDir) {
...
function destAbs(destination, path) {
  return join(destination, ...path.split("/"));
}
function inferredMutation(journal, intent, action) {
...
function inferIntent(destination, transactionDir, journal) {
  const intent = journal.intent;
  if (!intent) {
    return journal;
  }
...
function classifyRecovery(destination, transactionDir, journal) {
  const installPath = join(validateDestinationRoot(destination), ".deniz-skills", "install.json");
  try {
...
export function inspectRecovery(destination) {
  const root = validateDestinationRoot(destination);
  const deniz = join(root, ".deniz-skills");
  if (!existsLstat(deniz)) {
    return null;
...
function recordedIdentity(state, path) {
  const owned = state.files[path];
  if (!owned) {
    return null;
  }
...
function recheckOperation(destination, current, operation, platform, expectAbsent = false) {
...
function writeSnapshot(transactionDir, journal, seq) {
  const dir = snapshotsDir(transactionDir);
...
function bundleSourcePath(root, source) {
...
function ensureParents(destination, path, created) {
  const parts = path.split("/");
...
function stagedFilePath(transactionDir, path) {
  return join(transactionDir, "files", ...path.split("/"));
}
function backupFilePath(transactionDir, path) {
  return join(transactionDir, "backup", ...path.split("/"));
}
function unlinkManagedFile(destination, path, io) {
...
function restoreBackup(destination, transactionDir, path, created, expected) {
...
function pruneCreatedDirectories(destination, created) {
...
function pruneRemovedDirectories(destination, candidates) {
...
function requireTransactionStateEvidence(transactionDir, journal) {
...
function mutationActions(journal, operationIndex) {
  return new Set(
    journal.applied.filter((mutation) => mutation.operationIndex === operationIndex).map((mutation) => mutation.action),
  );
}
function observedMatchesIdentity(observed, expected) {
  return observed.kind === "file" && identityMatches(observed.identity, expected, hostPlatform());
}
function preflightRollbackStateEvidence(destination, transactionDir, journal, oldState) {
  const installPath = join(validateDestinationRoot(destination), ".deniz-skills", "install.json");
...
function preflightRollbackEvidence(destination, journal, oldState, newState) {
  for (const [operationIndex, operation] of journal.operations.entries()) {
...
function rollbackApplied(destination, transactionDir, journal, io) {
...
function verifyRollbackComplete(destination, transactionDir, journal) {
  const evidence = requireTransactionStateEvidence(transactionDir, journal);
  const installPath = join(validateDestinationRoot(destination), ".deniz-skills", "install.json");
  if (existsLstat(installPath)) {
    requireOrdinaryFile(installPath, "Install state");
...
function verifyFinalizeComplete(destination, transactionDir, journal) {
  const installPath = join(validateDestinationRoot(destination), ".deniz-skills", "install.json");
...
function requireBackupEvidence(transactionDir, journal) {
  const platform = hostPlatform();
  for (const mutation of journal.applied) {
    if (mutation.action !== "backed-up") {
      continue;
...
function removeTransactionDir(transactionDir) {
  requireOrdinaryDir(transactionDir, "transaction");
  rmSync(transactionDir, { recursive: true, force: true });
}
function stageBundleFile(transactionDir, bundles, operation) {
...
function verifyStagedFile(path, expected, managedPath) {
...
function deviceIdOf(path, io) {
  if (io?.deviceId) {
    return io.deviceId(path);
  }
  return lstatSync(path).dev;
}
function requireSameDevice(left, right, io) {
...
function requireManagedDevice(destination, managedPath, io) {
  requireSameDevice(destination, nearestExistingAncestor(destAbs(destination, managedPath)), io);
}
function requireDestinationTopology(destination, deniz, io, managedPaths = []) {
...
function renameOrThrow(from, to) {
  try {
    renameSync(from, to);
  } catch (error) {
    if (isEXDEV(error)) {
...
function stagedInstallStatePath(transactionDir) {
  return join(transactionDir, "staged-install.json");
}
function verifyStagedInstallState(transactionDir, journal) {
...
function commitInstallState(destination, transactionDir, journal, seq, options) {
  const installPath = join(validateDestinationRoot(destination), ".deniz-skills", "install.json");
  const staged = stagedInstallStatePath(transactionDir);
  const backed = join(transactionDir, "backup-install.json");
...
export function applyPlan(lock, destination, plan, bundles, options) {
...
export function applyRecovery(lock, destination, recovery, options) {
  requireHeldLock(lock, destination);
  const { root, deniz } = ensureDestinationTree(destination);
