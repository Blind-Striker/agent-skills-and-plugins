...
is which initiation paths are required or forbidden, what shape the artifact takes, and whether any
of it makes sense in a harness it was never written for. That is the product.
...
**Axis 1 — Invocation: which initiation capabilities are required or forbidden.**
`invocation: auto | manual | both`, per [ADR-0005](0005-invocation-intent-in-the-manifest.md).
`auto` requires implicit availability without forbidding a native explicit path, `manual` forbids
implicit selection and requires an explicit path, and `both` requires both. Upstream skills
overwhelmingly fire on their own; taking that back where a ceremony is required is the single
behaviour this repo exists for.
...
is therefore an input to resolution, never a binding authority. Axis 1 answers *which initiation
paths the result must or must not provide*, while axis 2 answers *what it is* — conflating them costs
the ability to say "a command that the model may also reach", where a target can express it.
...
The emitters resolve these axes independently into each harness's native invocation, shape, and fit.
Current mappings, sequencing, and output details are owned by
...
- `invocation` and `as` can disagree in useful ways and can also state combinations a harness cannot
  express. Emitters must not guess at required or forbidden capabilities; a capability the manifest
  leaves unspecified may follow the target's native surface.
- Shape and fit being per-harness means the output trees are intentionally not comparable; review
  has to inspect all three.
...
- Adapting content the emitter cannot reconcile mechanically implies per-harness body edits, which
  the overlay mechanism (ADR-0001) does not currently express — it produces one body for all three
