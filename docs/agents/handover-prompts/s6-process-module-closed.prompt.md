...
| **First pointer edge** | The review mix ships `/mattpocock-skills:setup-matt-pocock-skills` → `/deniz-process:…` (Claude) / `/…` (OpenCode). ADR-0008's pointer tier had **zero** shipped instances until now; it was enforced against fixtures only | Verified in both trees and in the ledger |
...
| **Parked-warning fix** (`defab5f`) | The build called every `both` item's live skill directory "parked". 8 of 14 warnings were false; build now agrees with the ledger exactly (6) | Report and ledger cross-checked to the same six names |
