...
description: Reference data for detecting the .NET test platform (VSTest vs
  Microsoft.Testing.Platform), test framework (MSTest, xUnit, NUnit, TUnit), and
  project system (SDK-style vs classic non-SDK/packages.config) from project
  files. DO NOT USE directly — loaded by run-tests and mtp-hot-reload when they
