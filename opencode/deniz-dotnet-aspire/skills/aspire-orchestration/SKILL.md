...
description: '**WORKFLOW SKILL** — Manage Aspire lifecycle in VS Code or the
  ...
  (aspire-deployment), logs/traces/metrics (aspire-monitoring), or
  AppHost code and resource wiring (aspireify). FOR SINGLE OPERATIONS:
...
| Scenario | Route To |
|----------|----------|
| AppHost wiring after `aspire init` (scan repo, add resources, ServiceDefaults/OTel) | → `aspireify` skill ([`aspireify`](../aspireify/SKILL.md)) or project-local `.agents/skills/aspireify/SKILL.md` |
...
validate via `aspire start`) to the in-plugin **aspireify** skill: [`aspireify`](../aspireify/SKILL.md).
For first-run flows that only need the skeleton drop, see the in-plugin **aspire-init** skill:
[`aspire-init`](../aspire-init/SKILL.md). This orchestration skill stays focused
