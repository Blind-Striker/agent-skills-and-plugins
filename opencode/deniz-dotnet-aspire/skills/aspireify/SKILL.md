...
description: "**WORKFLOW SKILL** - Wire an Aspire AppHost after `aspire init`
  ...
  USE FOR: skeleton drop (use aspire-init), start/stop/wait/restart (use
  aspire-orchestration), publish/deploy/destroy (use
  aspire-deployment), logs/traces (use aspire-monitoring).
...
the app → `aspire-orchestration`. If the user wants to **deploy** →
`aspire-deployment`.
