...
description: "**WORKFLOW SKILL** - Wire Aspire AppHosts or repair TypeScript
  ...
  or Yarn Classic. DO NOT USE FOR: skeleton drop (aspire-init),
  lifecycle-only start/stop/wait/restart or `aspire update --migrate`
  (aspire-orchestration), publish/deploy/destroy
  (aspire-deployment), logs/traces (aspire-monitoring). INVOKES:
