...
description: "**WORKFLOW SKILL** - Aspire 13.5.3 router. Detects AppHosts,
  ...
  non-Aspire projects or ordinary build/test tasks. INVOKES: aspire-init,
...
| Migrate legacy TypeScript `apphost.ts` (`aspire update --migrate`) | → [aspire-orchestration](../aspire-orchestration/SKILL.md); hand back to aspireify only if source authoring remains |
