...
description: "**WORKFLOW SKILL** - First-run flow for adding Aspire to a repo.
  ...
  AppHost skeleton, then hands off to `aspireify` for resource wiring.
  ...
  aspireify), start/stop/wait (use aspire-orchestration),
  deploy/publish (use aspire-deployment), logs/traces (use
  aspire-monitoring), repo that already has an AppHost. INVOKES: aspire
  CLI (init, new, doctor), aspireify (handoff after skeleton drop). FOR
