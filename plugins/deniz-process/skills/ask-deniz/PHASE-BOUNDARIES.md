...
| **Clear** (`/clear` in Claude Code, `/new` in OpenCode, or a new Codex chat/session) | Empty the context window and start from nothing. |
| **`/deniz-process:handoff`** | Write a portable markdown file and seed a session anywhere with it. |
...
**2. Is the context irrelevant to what comes next?** Is everything in this session (the exploration, the decisions, the dead ends) disposable? If so, **clear the window** — `/clear` in Claude Code, `/new` in OpenCode, or start a new Codex chat or CLI session. It is the cheapest move on the board: it takes no time and hands back the whole window. Clearing also is not terminal: the old session stays resumable.
...
- swapping to a **new harness** (Claude Code → OpenCode),
