...
| **Clear** (`/clear` in Claude Code, `/new` in OpenCode) | Empty the context window and start from nothing. |
...
**2. Is the context irrelevant to what comes next?** Is everything in this session (the exploration, the decisions, the dead ends) disposable? If so, **clear the window** — `/clear` in Claude Code, `/new` in OpenCode. It is the cheapest move on the board: it takes no time and hands back the whole window. Clearing also is not terminal: the old session stays resumable.
