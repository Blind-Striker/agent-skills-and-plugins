...
description: "**ANALYSIS SKILL** - Observe Aspire apps: logs, traces, metrics,
  ...
  aspire dashboard run, --include-hidden, browser logs in dashboard,
  WithBrowserLogs, App Insights query, AKS pod logs, container app logs. DO NOT
  USE FOR: start/stop/wait (use deniz-dotnet-aspire:aspire-orchestration),
  deploy/publish/destroy (use deniz-dotnet-aspire:aspire-deployment), AppHost code edits like
  WithBrowserLogs() (use deniz-dotnet-aspire:aspireify), Azure provisioning (use
  azure-prepare). INVOKES: aspire CLI, azure-diagnostics (deployed Azure),
