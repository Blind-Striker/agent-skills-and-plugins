...
description: "**ANALYSIS SKILL** - Observe Aspire apps: logs, traces, metrics,
  ...
  deployed Azure resource health. USE FOR: aspire logs, aspire otel logs, aspire
  otel traces, aspire otel spans, aspire describe, aspire ps, aspire export,
  aspire dashboard run, describe --include-hidden, browser logs in dashboard,
  terminal view, DevTunnel URLs, WithBrowserLogs, App Insights query, AKS pod
  logs, container app logs. DO NOT USE FOR: start/stop/wait (use
  deniz-dotnet-aspire:aspire-orchestration), deploy/publish/destroy (use
  deniz-dotnet-aspire:aspire-deployment), AppHost code edits like WithBrowserLogs() (use
  deniz-dotnet-aspire:aspireify), Azure provisioning (use azure-prepare). INVOKES: aspire
  CLI, azure-diagnostics (deployed Azure), kubectl + Container Insights. FOR
  SINGLE OPERATIONS: Run the aspire CLI command directly for quick log or
  describe lookups."
license: MIT
metadata:
  author: Microsoft
...
| AppHost / deployment definition | Authoring | deniz-dotnet-aspire:aspire-deployment skill | → `deniz-dotnet-aspire:aspire-deployment` skill |
...
1. Is this about **AppHost code or deployment definition**? → `deniz-dotnet-aspire:aspire-deployment` skill.
...
| Add `WithBrowserLogs()` to a resource | → **`deniz-dotnet-aspire:aspireify` skill** (AppHost authoring) — do not edit the AppHost from this skill |
...
> Authoring custom commands or `WithBrowserLogs()` calls is AppHost work — route to **`deniz-dotnet-aspire:aspireify`**. This skill is for *observing* what those features surface in the dashboard.
...
| Scenario | Route To |
|----------|----------|
| Start/stop/wait/rebuild lifecycle | → `deniz-dotnet-aspire:aspire-orchestration` skill |
| Deploy, publish, pipeline steps, AppHost compute environment binding | → `deniz-dotnet-aspire:aspire-deployment` skill |
| AppHost code changes (`WithBrowserLogs()`, custom commands, `WithHttpCommand`) | → `deniz-dotnet-aspire:aspireify` skill |
