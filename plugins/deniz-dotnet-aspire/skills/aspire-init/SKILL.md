...
description: "**WORKFLOW SKILL** - First-run flow for adding Aspire to a repo.
  ...
  Aspire, no AppHost detected, install aspireify, generated .aspire/modules. DO
  NOT USE FOR: AppHost wiring on an existing AppHost (use deniz-dotnet-aspire:aspireify),
  start/stop/wait (use deniz-dotnet-aspire:aspire-orchestration), deploy/publish (use
  deniz-dotnet-aspire:aspire-deployment), logs/traces (use deniz-dotnet-aspire:aspire-monitoring), repo
  that already has an AppHost. INVOKES: aspire CLI (init, new, doctor),
  deniz-dotnet-aspire:aspireify (handoff after skeleton drop). FOR SINGLE OPERATIONS: Run
  `aspire init` or `aspire new TEMPLATE` directly."
license: MIT
metadata:
  author: Microsoft
  version: 0.0.1
