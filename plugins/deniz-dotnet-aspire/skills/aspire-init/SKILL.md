...
description: "**WORKFLOW SKILL** - First-run flow for adding Aspire to a repo.
  Picks `aspire new` (greenfield) or `aspire init` (existing repo), drops the
  AppHost skeleton, then hands off to `deniz-dotnet-aspire:aspireify` for resource wiring.
  USE FOR: aspire init, aspire new, aspire-starter, aspire-ts-starter,
  aspire-py-starter, add Aspire to existing repo, scaffold Aspire app, bootstrap
  Aspire, no AppHost detected, install aspireify, apphost.mts, generated
  .aspire/modules. DO NOT USE FOR: AppHost wiring on an existing AppHost (use
  deniz-dotnet-aspire:aspireify), start/stop/wait (use deniz-dotnet-aspire:aspire-orchestration),
  deploy/publish (use deniz-dotnet-aspire:aspire-deployment), logs/traces (use
  deniz-dotnet-aspire:aspire-monitoring), repo that already has an AppHost. INVOKES: aspire
  CLI (init, new, doctor), deniz-dotnet-aspire:aspireify (handoff after skeleton drop). FOR
  SINGLE OPERATIONS: Run `aspire init` or `aspire new TEMPLATE` directly."
license: MIT
metadata:
  author: Microsoft
  version: 0.0.2
...
> [`deniz-dotnet-aspire:aspireify`](../aspireify/SKILL.md) for the actual resource wiring.
...
[`deniz-dotnet-aspire:aspireify`](../aspireify/SKILL.md) (re-wire) or
[`deniz-dotnet-aspire:aspire-orchestration`](../aspire-orchestration/SKILL.md) (lifecycle).
...
5. Route to [`deniz-dotnet-aspire:aspire-orchestration`](../aspire-orchestration/SKILL.md) for first run
   (`aspire start`).
...
5. After `aspireify` finishes wiring, validate via `aspire start`
   ([`deniz-dotnet-aspire:aspire-orchestration`](../aspire-orchestration/SKILL.md)).
...
|------------------------------------------------|----------|
| Skeleton dropped, resources need wiring | → `deniz-dotnet-aspire:aspireify` skill (in-plugin or project-local) |
| Skeleton dropped, validate it starts | → `deniz-dotnet-aspire:aspire-orchestration` (run `aspire start`) |
| New project from template, ready to run | → `deniz-dotnet-aspire:aspire-orchestration` |
| User asks to deploy after init | → `deniz-dotnet-aspire:aspire-deployment` |
| User asks for logs/traces after init | → `deniz-dotnet-aspire:aspire-monitoring` |
| Existing AppHost detected — do NOT run init | → `deniz-dotnet-aspire:aspireify` (re-wire) or `deniz-dotnet-aspire:aspire-orchestration` (lifecycle) |
...
|---------|-------|--------|
| `aspire init` reports AppHost already exists | Repo already has an AppHost | Stop. Route to `deniz-dotnet-aspire:aspireify` (re-wire) or `deniz-dotnet-aspire:aspire-orchestration` (lifecycle) |
...
| Skeleton dropped but resources not wired | Expected — `aspire init` does not wire | Hand off to `deniz-dotnet-aspire:aspireify` |
| Existing TypeScript AppHost still uses `apphost.ts` | Legacy entry point and package graph | Hand off to `deniz-dotnet-aspire:aspire-orchestration`, which owns approval and `aspire update --migrate --yes --non-interactive`; return to deniz-dotnet-aspire:aspireify only for later source authoring |
