...
| AppHost already exists in the repo | `deniz-dotnet-aspire:aspireify` (re-wire) or `deniz-dotnet-aspire:aspire-orchestration` (lifecycle) |
| Need to add an integration to an existing AppHost | `aspire add <package>` (handled by `deniz-dotnet-aspire:aspire-orchestration`) |
