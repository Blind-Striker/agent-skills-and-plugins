...
1. **Pre-flight** — verify no AppHost already exists. If one does, stop and route to
   `deniz-dotnet-aspire:aspireify` or `deniz-dotnet-aspire:aspire-orchestration`.
2. **Run init**:
   ```bash
...
6. **Validate** — once `aspireify` finishes wiring, run `aspire start` (handled by
   `deniz-dotnet-aspire:aspire-orchestration`) and confirm resources reach a healthy state.
...
|---------|-------|----------|
| `aspire init` reports an AppHost already exists | Repo is already an Aspire app | Stop. Route to `deniz-dotnet-aspire:aspireify` or `deniz-dotnet-aspire:aspire-orchestration` |
