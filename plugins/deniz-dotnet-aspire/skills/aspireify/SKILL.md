...
description: "**WORKFLOW SKILL** - Wire an Aspire AppHost after `aspire init`
  drops a skeleton. Scans the repo, proposes a resource graph, edits the AppHost
  (C#, file-based C#, or TypeScript), wires `Aspire.ServiceDefaults` + OTel,
  validates with `aspire start`, then self-deactivates. USE FOR: wire AppHost,
  scaffold resource graph, add Postgres/Redis/Rabbit/Mongo to Aspire, connect
  frontend to API, after `aspire init` what next, AddNextJsApp, AddViteApp,
  WithBrowserLogs, file-based apphost.cs, apphost.ts, unified withEnvironment,
  refuse .aspire/modules edit, migrate .env files, migrate user secrets. DO NOT
  USE FOR: skeleton drop (use deniz-dotnet-aspire:aspire-init), start/stop/wait/restart (use
  deniz-dotnet-aspire:aspire-orchestration), publish/deploy/destroy (use
  deniz-dotnet-aspire:aspire-deployment), logs/traces (use deniz-dotnet-aspire:aspire-monitoring).
  INVOKES: aspire CLI (add, start, wait, describe, docs api search, stop),
  AppHost source edits, ServiceDefaults wiring. FOR SINGLE OPERATIONS: Run
  `aspire add PACKAGE` directly for a one-off integration."
license: MIT
metadata:
  author: Microsoft
  version: 0.0.1
...
the app → `deniz-dotnet-aspire:aspire-orchestration`. If the user wants to **deploy** →
...
5. DEACTIVATE → confirm clean start, hand off to deniz-dotnet-aspire:aspire-orchestration
```
...
```
✅ AppHost wired and validated. Handing off to deniz-dotnet-aspire:aspire-orchestration for
...
| File-lock errors during edit | Hand off to `deniz-dotnet-aspire:aspire-orchestration` → `aspire stop` → retry |
...
| Scenario | Route To |
|----------|----------|
| AppHost skeleton not yet dropped | → `deniz-dotnet-aspire:aspire-init` skill |
| Day-to-day start/stop/wait/restart | → `deniz-dotnet-aspire:aspire-orchestration` skill |
| Publish, deploy, destroy, pipeline steps | → `deniz-dotnet-aspire:aspire-deployment` skill |
| Logs, traces, metrics, dashboard, browser log inspection | → `deniz-dotnet-aspire:aspire-monitoring` skill |
