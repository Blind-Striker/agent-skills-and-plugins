...
and hands off to `deniz-dotnet-aspire:aspire-orchestration`.
...
| File-lock error during edit | Hand off to `deniz-dotnet-aspire:aspire-orchestration` → `aspire stop` → retry edit |
...
Handing off to deniz-dotnet-aspire:aspire-orchestration for day-to-day start/stop/wait.
Aspireify is done.
```
...
Then route them to `deniz-dotnet-aspire:aspire-deployment` for the actual `aspire deploy` /
