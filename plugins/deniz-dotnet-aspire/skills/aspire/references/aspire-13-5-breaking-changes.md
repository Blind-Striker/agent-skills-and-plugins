...
- `aspire update --migrate` updates the project's Aspire packages first, then migrates
  ...
  the package update and migration. `deniz-dotnet-aspire:aspire-orchestration` owns this CLI migration;
  `deniz-dotnet-aspire:aspireify` owns only any AppHost source authoring that remains afterward.
