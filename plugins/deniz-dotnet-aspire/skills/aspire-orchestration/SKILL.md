...
description: '**WORKFLOW SKILL** — Manage Aspire lifecycle in VS Code or the
  ...
  "--include-hidden", integration discovery, default watch, or hot reload.
  INVOKES: VS Code lifecycle tools first when exposed; Aspire CLI for readiness,
  inspection, resource operations, isolated worktree starts, and allowed
  fallbacks. DO NOT USE FOR: deploy/publish/destroy (deniz-dotnet-aspire:aspire-deployment),
  logs/traces/metrics (deniz-dotnet-aspire:aspire-monitoring), or AppHost code and resource
  wiring (deniz-dotnet-aspire:aspireify). FOR SINGLE OPERATIONS: Load the matching editor
  tool first, then obey the exact-target, worktree-isolation, and stop-result
  rules below.'
license: MIT
metadata:
  author: Microsoft
...
| Custom resource commands (`WithCommand`, `ExecuteCommandResult`, `HttpCommandResultMode`) | → `deniz-dotnet-aspire:aspireify` skill |
...
Detection covers TS AppHosts (`apphost.ts`), but **all TS AppHost authoring is delegated to `deniz-dotnet-aspire:aspireify`**.
