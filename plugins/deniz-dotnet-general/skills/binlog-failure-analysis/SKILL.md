...
description: "Analyze MSBuild binary logs to diagnose build failures. USE FOR:
  build errors that are unclear from console output, diagnosing cascading
  failures across multi-project builds, tracing MSBuild target execution order,
  and generally any MSBuild build issues. Requires an existing .binlog file. DO
  NOT USE FOR: generating binlogs (use binlog-generation), non-MSBuild build
  systems."
license: MIT
...
This skill diagnoses MSBuild build failures from a `.binlog` file. The portable
path is the **binlog replay** workflow below — it works everywhere the .NET SDK
does. If the **binlog MCP server** (`Microsoft.AITools.BinlogMcp`, exposed under
the `binlog` MCP namespace) is configured in your environment, prefer it: it
queries the binlog directly without replaying.
