...
description: "Diagnose MSBuild build performance bottlenecks using binary log
  analysis. USE FOR: identifying why builds are slow by analyzing binlog
  performance summaries, detecting ResolveAssemblyReference (RAR) taking >5s,
  Roslyn analyzers consuming >30% of Csc time, single targets dominating >50% of
  build time, node utilization below 80%, excessive Copy tasks, NuGet restore
  running every build. Covers timeline analysis, Target/Task Performance Summary
  interpretation, and 7 common bottleneck categories. Use after
  build-perf-baseline has established measurements. DO NOT USE FOR: establishing
  initial baselines (use build-perf-baseline first), fixing incremental build
  issues (use incremental-build), parallelism tuning (use build-parallelism),
...
2. If the **binlog MCP server** (`Microsoft.AITools.BinlogMcp`, exposed under the `binlog` MCP namespace) is configured in your environment, use it to query the binlog directly; otherwise follow the portable replay flow below.

### Portable flow: binlog replay to text logs
...
### 8. ResolveProjectReferences Looks Expensive but Is Not the Bottleneck

- **Symptom**: `ResolveProjectReferences` tops the **Target** Performance Summary, sometimes at 50-80% of build time
- **Why it misleads**: the reported time is wall-clock **wait time for dependent projects** while the MSBuild node is yielded (dotnet/msbuild#3135) — the node may be doing useful work elsewhere; the target itself does very little
- **Do not** set `ResolveProjectReferences` as the optimization target
- **Instead**: read the **Task** Performance Summary and optimize task self-time — `Csc` (section 2), `ResolveAssemblyReference` (section 1), `Copy` (section 4); serialization between projects is covered by the `build-parallelism` skill
