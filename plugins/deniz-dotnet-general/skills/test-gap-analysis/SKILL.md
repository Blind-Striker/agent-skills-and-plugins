...
description: "Performs report-only pseudo-mutation analysis of existing tests:
  find caller-visible production-code changes that current assertions would not
  catch. Use when asked whether a bug or mutation could survive, where tests are
  behaviorally blind, or which production outcomes lack protection. Polyglot. DO
  NOT USE FOR: writing tests, suite organization or taxonomy, coverage/CRAP
  interpretation, assertion/smell audits, or running a mutation-testing tool."
license: MIT
...
| Add or create tests | Report the verified gaps; test writing is outside this skill |
...
explicit verification or an exhaustive audit.
...
Outside explicit verification or an exhaustive audit, execute no mutations. Do
not mutate to confirm obvious no coverage.
...
Enter this phase only for explicit verification or an exhaustive audit.
