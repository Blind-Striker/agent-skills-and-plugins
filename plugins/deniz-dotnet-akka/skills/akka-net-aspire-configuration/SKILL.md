...
description: Configure Akka.NET with .NET Aspire for local development and
  production deployments. Covers actor system setup, clustering, persistence,
  Akka.Management integration, and Aspire orchestration patterns.
invocable: false
...
- **`deniz-dotnet-akka:akka-net-management`** - Deep dive into Akka.Management, Cluster Bootstrap, and discovery providers (Kubernetes, Azure, Config)
- **`deniz-dotnet-general:microsoft-extensions-configuration`** - IValidateOptions patterns for configuration validation
- **`deniz-dotnet-akka:akka-net-best-practices`** - Cluster/local mode abstractions for testable actor systems
- **`deniz-dotnet-aspire:aspire-integration-testing`** - Testing Aspire applications with real infrastructure

## Core Principles

1. **Configuration via Microsoft.Extensions.Configuration** - Use strongly-typed settings classes bound from appsettings.json (see `deniz-dotnet-general:microsoft-extensions-configuration` skill)
...
For comprehensive Akka.NET testing patterns using **Akka.Hosting.TestKit**, see the `deniz-dotnet-akka:akka-net-testing-patterns` skill.
...
When testing Aspire applications with Akka.NET actors, combine `deniz-dotnet-aspire:aspire-integration-testing` patterns with `deniz-dotnet-akka:akka-net-testing-patterns`:
...
**For unit testing individual actors**, use `deniz-dotnet-akka:akka-net-testing-patterns` with in-memory persistence (no Aspire needed).
