...
description: '**WORKFLOW SKILL** — Deploy Aspire apps from AppHost models to
  Docker Compose, Kubernetes, Azure, AWS, or preview Radius. WHEN: "deploy
  Aspire app", "publish Aspire artifacts", "deploy to Azure Container Apps",
  "generate Kubernetes artifacts", "deploy to Radius", "tear down Aspire
  deployment". INVOKES: aspire CLI, Aspire docs, target cloud/container CLIs.
  FOR SINGLE OPERATIONS: use generic Azure, Kubernetes, Docker, AWS, or Radius
  tools only when no Aspire AppHost exists.'
license: MIT
metadata:
  author: Microsoft
  version: 0.0.2
...
1. **Orient to the Aspire workspace.**
   ...
   - If no AppHost exists, stop deployment work and invoke the `$deniz-dotnet-aspire:aspireify` skill to initialize/wire the AppHost before continuing.
...
- [aspire-13-5-breaking-changes.md](../aspire/references/aspire-13-5-breaking-changes.md) - 13.5.3 package, API, and CLI migration rules.
...
| Scenario | Route To |
|----------|----------|
| Start, stop, wait, or restart the AppHost / its resources | `$deniz-dotnet-aspire:aspire-orchestration` skill |
| Logs, traces, metrics, dashboard for a running or deployed app | `$deniz-dotnet-aspire:aspire-monitoring` skill |
| AppHost authoring — adding integrations, wiring resources, environment setup | `$deniz-dotnet-aspire:aspireify` skill |
