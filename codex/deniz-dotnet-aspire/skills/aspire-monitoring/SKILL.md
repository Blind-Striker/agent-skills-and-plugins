...
description: "**ANALYSIS SKILL** - Observe Aspire apps: logs, traces, metrics,
  ...
  $deniz-dotnet-aspire:aspire-deployment), AppHost code edits like
  WithBrowserLogs() (use $deniz-dotnet-aspire:aspireify), Azure provisioning
  (use azure-prepare). INVOKES: aspire CLI, azure-diagnostics (deployed Azure),
  kubectl + Container Insights. FOR SINGLE OPERATIONS: Run the aspire CLI
