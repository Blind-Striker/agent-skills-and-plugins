...
how to customize it), `overlays/` (body edits — a patch, or files we own
outright) and `skills/`
(original skills). `plugins/`, `opencode/`, and `dist/` are build output —
committed so a clone and packed installer work immediately, regenerated by
`npm run build`, and never edited.
...
mounts. See the [adapter guide](docs/research/harness-adapters.md) for the harness boundary and the
[experiment protocol](experiments/harness-invocation/protocol.md) for isolated verification.
